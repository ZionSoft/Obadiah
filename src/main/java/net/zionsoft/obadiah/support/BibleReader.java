...
class BibleReader {
        ...
        try {
            ...
            for (int i = 0; i < length; ++i) {
                ...
                if (booksInfoObject.has("shortName")) {
                    ...
                    } else if (name.equals("Italian Deodati Bibbia")
                            || name.equals("Italian Diodati Bibbia")) {
            ...
            if (count > 0) {
                mInstalledTranslations = new TranslationInfo[count];
                for (int i = 0, index = 0; i < length; ++i) {
                    if (translations[i] != null)
                        mInstalledTranslations[index++] = translations[i];
                }
            } else {
                mInstalledTranslations = null;
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    TranslationInfo[] installedTranslations() {
        return mInstalledTranslations;
    }

    void selectTranslation(String translation) {
        if (mInstalledTranslations == null || mInstalledTranslations.length == 0) {
            mSelectedTranslation = null;
            return;
        }

        if (translation == null) {
            if (mSelectedTranslation == null)
                mSelectedTranslation = mInstalledTranslations[0].path;
            return;
        }

        // tries to find the translation
        for (TranslationInfo installedTranslation : mInstalledTranslations) {
            if (installedTranslation.path.endsWith(translation)) {
                mSelectedTranslation = installedTranslation.path;
                return;
            }
        }
